...
export default function EmployeeOnboarding() {
  ...
  useEffect(() => {
    ...
    if (editMode && editId && tenantId) {
      const fetchEditData = async () => {
        try {
          const { getEmployeeById } = await import('@/services/employeeService');
          const res = await getEmployeeById(editId, tenantId);
          const emp = res.data;

          // Determine if sameAsCurrent should be true
          const isSameAddress = emp.currentStreet === emp.permanentStreet &&
            emp.currentCity === emp.permanentCity &&
            emp.currentState === emp.permanentState &&
            emp.currentZip === emp.permanentZip &&
            emp.currentStreet !== '';
          ...
          setFormData({
            ...
            leave: {
              ...
              workingDays: Array.isArray(emp.workingDays) ? emp.workingDays : [],
            },
            documents: {
              identity: emp.identityProofUrl ? { url: emp.identityProofUrl } : null,
              education: emp.educationCertUrl ? { url: emp.educationCertUrl } : null,
              employment: emp.employmentProofUrl ? { url: emp.employmentProofUrl } : null,
            }
          });
        } catch (e) {
  ...
  const transformEmployeePayload = (data) => {
    ...
    return {
      ...
      designation: data.job.designation || data.personal.designation,
      department: data.job.department || data.personal.department,
      role: data.job.role,
      ...
      // ================= SALARY =================
      annualCtc: cleanNumber(data.salary.annualPackage),
      monthlyGross: cleanNumber(data.salary.monthlyGross),
      basicSalary: cleanNumber(data.salary.basicSalary),
      performanceBonus: cleanNumber(data.salary.performanceBonus),
      professionalTax: cleanNumber(data.salary.professionalTax),
    ...
    if (currentIndex < tabs.length - 1) {
      ...
      try {
        ...
        if (editMode && editId) {
          ...
          await updateEmployee(editId, payload, tenantId);
          console.log('Updating Employee Data:', formData);
          alert('Employee Profile Updated Successfully!');
        } else {
          const { createEmployee } = await import('@/services/employeeService');
          await createEmployee(payload, tenantId);
        ...
        route.push(`/${params.dashboard}/admin/employeemanagement/employee-list`);
      } catch (error) {
  ...
  const checkTabValidity = (tabId) => {
    switch (tabId) {
      ...
      case 'contact': {
        const currentValid = Object.values(formData.contact.current).every(v => v && v.trim() !== '');
        const emergencyValid = Object.values(formData.contact.emergency).every(v => v && v.trim() !== '');

        // Only validate fields present in the Permanent Address form (Street, City, State)
        const perm = formData.contact.permanent;
        const permanentValid = [perm.street, perm.city, perm.state].every(v => v && v.trim() !== '');
