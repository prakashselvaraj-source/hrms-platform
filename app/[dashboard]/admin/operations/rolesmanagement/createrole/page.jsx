...
import { ChevronLeft, ChevronRight, Shield, Users, Lock, Save, Loader2, Info, ShieldCheck, Unlock, ShieldAlert } from "lucide-react";
...
export default function CreateRolePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createRole(payload, tenantId);
      router.back();
    } catch (err) {
      console.error(err);
    } finally {
      ...
      <div className="max-w-[800px] mx-auto px-4 sm:px-8 py-10">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-4">
            ...
            <ChevronRight size={12} className="opacity-40" />
            <span className="text-indigo-600">Create Entity</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">Initialize New Role</h1>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2 opacity-70">Defining baseline access protocols</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-8">
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                placeholder="e.g. Strategic Operations Lead"
        ...
        <div className="mt-8 p-6 bg-indigo-50/50 backdrop-blur-sm border border-indigo-100 rounded-2xl flex gap-4">
          ...
          <p className="text-[11px] text-indigo-900 font-medium leading-relaxed">
            <span className="font-black uppercase tracking-widest block mb-1">Post-Initialization Note</span>
            Upon role creation, specific module-level permissions (View, Create, Edit, Delete) can be surgicaly calibrated from the primary Roles dashboard.
          </p>
        </div>
      </div>
    </div>
