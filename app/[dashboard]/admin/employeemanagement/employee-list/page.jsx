    ...
    <tr className="border-b border-[#F1F5F9] transition-colors duration-100">
      ...
      <td className="px-4 py-3.5">
        <div className="flex items-center gap-2">
          <button id={`view-${employee.id}`} aria-label="View" className="w-7 h-7 flex items-center justify-center rounded-lg text-[#4A45B6] transition-colors hover:bg-gray-100" onClick={() => onView(employee.id)} >
            <EyeIcon />
          </button>
          <button id={`edit-${employee.id}`} aria-label="Edit" className="w-7 h-7 flex items-center justify-center rounded-lg text-[#4A45B6] transition-colors hover:bg-gray-100" onClick={() => onEdit(employee.id)}>
            <EditIcon />
          </button>
          <button id={`delete-${employee.id}`} aria-label="Delete" className="w-7 h-7 flex items-center justify-center rounded-lg text-[#BA1A1A] transition-colors hover:bg-gray-100" onClick={() => onDelete(employee.id)}>
            <TrashIcon />
          </button>
        </div>
      </td>
...
function DeleteConfirmationModal({ isOpen, onClose, onConfirm, name }) {
  useEffect(() => {
...
export default function EmployeesPage() {
  ...
  // currentPage is 0-based: page 0, 10 rows → "Showing 1–10 of 11"
  const from = totalEntries === 0 ? 0 : currentPage * rowsPerPage + 1;
  const to = Math.min((currentPage + 1) * rowsPerPage, totalEntries);
  // Sliding window of page numbers (0-based) around current page
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        const res = await getEmployees(tenantId, currentPage, rowsPerPage);
  ...
  const confirmDelete = async () => {
    if (!deletingEmployee || !tenantId) return;
    try {
      await deleteEmployee(deletingEmployee.id, tenantId);
  ...
  const handleEdit = (id) => {
    route.push(`/${params.dashboard}/admin/employeemanagement/add-employee?edit=true&id=${id}`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F2F4F6] p-4 sm:p-6">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 py-3 border-t border-[#F1F5F9]">
            ...
            <div className="flex items-center gap-1">
              ...
              <button id="next-page-btn" onClick={() => setCurrentPage((p) => Math.min(totalPages - 1, p + 1))} disabled={currentPage === totalPages - 1}
                className="w-7 h-7 flex items-center justify-center rounded-md border border-[#E2E8F0] text-[#434655] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                <ChevronRight />
              </button>
            </div>
        ...
        <AnimatePresence>
          {deleteModalOpen && deletingEmployee && (
            <DeleteConfirmationModal
              isOpen={deleteModalOpen}
              onClose={() => { setDeleteModalOpen(false); setDeletingEmployee(null); }}
              onConfirm={confirmDelete}
              name={deletingEmployee.firstName}
            />
          )}
        </AnimatePresence>
