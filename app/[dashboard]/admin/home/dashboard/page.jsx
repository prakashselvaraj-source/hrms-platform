...
export default function HRDashboard() {
  ...
  const birthdays = [
    { initials: "SJ", name: "Sarah Jenkins", date: "Tomorrow, Oct 24th" },
    { initials: "MR", name: "Michael Roberts", date: "Thu, Oct 26th" },
  ];
        ...
        <div className="flex items-center justify-between max-w-9xl mx-auto">
          <div className="hidden sm:flex">
            {["Overview", "Dashboard"].map((tab) => (
              <button key={tab}
                onClick={() => { setActiveTab(tab); router.push(tab === "Overview" ? "/home/overview" : `/home/${tab.toLowerCase()}`); }}
      ...
      <div className="px-4 sm:px-8 py-6 sm:py-8 max-w-9xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
          ...
          <div className="rounded-2xl p-6"
            ...
            <SectionHeader icon={<MegaphoneIcon />} title="Announcements" action="All Updates" />
            <div className="flex flex-col gap-3">
              {announcements.map((a, i) => (
                <div key={i} className="group rounded-xl overflow-hidden transition-all hover:shadow-sm cursor-pointer"
                  ...
                  <div className="h-1" style={{ backgroundColor: a.borderColor }} />
                  <div className="p-3.5">
          ...
          <div className="rounded-2xl overflow-hidden flex flex-col gap-3">
            {birthdays.map((b, i) => (
              <div key={i} className="flex-1 rounded-2xl p-5 flex flex-col justify-between min-h-[140px] relative overflow-hidden"
                style={{
                  background: `linear-gradient(135deg, var(--gradient-brand-from) 0%, var(--gradient-brand-to) 100%)`,
                }}>
                {/* BG decoration */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[14px] font-bold text-white leading-none">{b.name}</p>
                    <p className="text-[10px] mt-0.5" style={{ color: "var(--gradient-brand-sublabel)" }}>{b.date}</p>
                  </div>
                </div>

                <button
                  className="mt-3 text-[10px] font-bold uppercase tracking-[0.06em] text-white rounded-lg px-3 py-2 w-full transition cursor-pointer border-0"
                  style={{ backgroundColor: "var(--gradient-brand-btn)" }}
                  onMouseEnter={e => e.currentTarget.style.backgroundColor = "var(--gradient-brand-btn-hover)"}
                  onMouseLeave={e => e.currentTarget.style.backgroundColor = "var(--gradient-brand-btn)"}>
                  🎉 Wish Happy Birthday
