...
function Header() {
  ...
  const tabs = [
    { label: "Apply for Leave", link: `/${tenant}/leaveManagement` },
    { label: "Upcoming Holidays", link: `/${tenant}/leaveManagement/upcomingHolidays` },
    { label: "Leave Request status", link: `/${tenant}/leaveManagement/leaverequeststatus` },
  ];
          ...
          <div className="absolute opacity-0 pointer-events-none flex w-full overflow-hidden">
            {tabs.map((tab, i) => (
              <div
                ...
                className="px-3 py-2 whitespace-nowrap text-sm font-medium cursor-pointer"
              >
                {tab.label}
              </div>
            ))}
