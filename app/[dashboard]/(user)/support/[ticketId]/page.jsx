...
export default function TicketDetail() {
  ...
  useEffect(() => {
    ...
    const fetchTicket = async () => {
      try {
        const res = await getTicketById(tenant, params.ticketId);
        setTicket(res.data);
      } catch (err) {
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-100 px-2 py-1 rounded">
                  {ticket.messages?.length || 0} Update{(ticket.messages?.length !== 1) ? 's' : ''}
                </span>
              </div>
                  ...
                  <div className="flex gap-4 items-start">
                    ...
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="flex justify-end gap-3">
                        ...
                        <button className="bg-slate-900 hover:bg-black text-white px-6 py-2.5 rounded-xl text-xs font-bold transition-all active:scale-95 shadow-sm">
                          Post Update
                        </button>
                      </div>
                    </div>
